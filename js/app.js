...
/* ── 丸をつけた問題だけ ──
   本体の48問から MARU12_IDS の順に抜き出す。単元・出題形式の絞り込みは通さない。 */
const MARU12 = MARU12_IDS
  .map(id => Object.values(QUIZ_DATA).flat().find(q => q.id === id))
  .filter(Boolean);
...
$('maru-sub').textContent = `問題集で丸をつけた${MARU12.length}問`;
$('maru-btn').textContent = `この${MARU12.length}問を解く →`;
